 ...
 * bg-brand = #0D2C40 — never flips in dark/light mode.
 * SVG brand colours are hardcoded on purpose (physical scene rule).
 */
export function AuthHeader({ icon, title, subtitle, topLeft }: AuthHeaderProps) {
  return (
    <View className="relative overflow-hidden px-6 pb-14 pt-14 bg-brand">
      ...
      >
        <Svg width="100%" height="100%" viewBox="0 0 390 220" preserveAspectRatio="xMidYMid slice">
          ...
          <Polygon points="28,10 14,42 26,42 16,72 36,32 22,32"
            fill="none" stroke="#2272A6" strokeWidth="1" opacity="0.18" />

          {/* Lightning bolt — top right */}
          <Polygon points="295,18 288,34 295,34 287,52 300,28 293,28"
            fill="none" stroke="#2272A6" strokeWidth="0.8" opacity="0.25" />

          {/* Sine wave — bottom band */}
          <Path
            ...
            fill="none" stroke="#2272A6" strokeWidth="1" opacity="0.22"
          />
          <Path
            ...
            fill="none" stroke="#1A5A88" strokeWidth="0.7" opacity="0.18"
          />

          {/* Power plug icon — bottom left */}
          <Path
            d="M38 170 L38 182 M44 170 L44 182 M35 182 L47 182 L47 188 Q41 196 35 188 L35 182 Z"
            fill="none" stroke="#2272A6" strokeWidth="1"
          ...
          <Ellipse cx="370" cy="210" rx="60" ry="40"
            fill="none" stroke="#2272A6" strokeWidth="0.5" opacity="0.22" />
      ...
      {/* Icon badge
          size-14 = 56px, bg-primary/13 = 13% opacity  (config › opacity.13)
          border-primary/25 = 25% opacity border                            */}
      <View className="mb-5 w-14 h-14 items-center justify-center rounded-2xl bg-primary/13 border border-primary/25">
        {icon}
      </View>

      {/* Title — fontSize: 28 has no Tailwind step (2xl=24, 3xl=30), kept as style */}
      <Text style={{ color: '#F0F6FF', fontSize: 28, fontWeight: '700', fontFamily: 'Poppins-Bold' }}>
        {title}
      </Text>

      {/* Subtitle — text-base = 16px closest to 15px, or keep style for exact match */}
      <Text style={{ color: '#8BA8C4', fontSize: 15, marginTop: 4, fontFamily: 'Poppins' }}>
        {subtitle}
      </Text>
    </View>
  );
